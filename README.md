...
| Password traversal detection | **Account Takeover** | Using the same username, e.g. joe, and login with different passwords tens of times until the api call is blocked with 403 response code. If later on, you need to login, but your IP is still blocked, you can put the rule in Count mode temporarily. |
...
| **Caching** | Verify that static page resources (e.g. javascript / css files) are served from CloudFront cache, using the following header ```X-Cache: Hit from cloudfront```. If the result is a miss, reload the page and verify that you get a cache hit. Verify that the html for the home page also results in a cache hit, when no user is logged. In this case, the server returns a ```stale-while-revalidate``` directive in the ```Cache-Control``` header. <br/> Now, log in (user: Joud, pwd: demo), reload the home page, and verify that the logged experience is not cached a ```private, no-cache, no-store, max-age=0, must-revalidate``` directive in the ```Cache-Control``` header.|
